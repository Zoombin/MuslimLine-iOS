...
class ReadViewCell: UITableViewCell {
    ...
    func calculateHeight(quran :Quran) {
            if (quran.hasCalulateHeight == false) {
                ...
                textCn.frame = CGRectMake(textQuran.frame.origin.x, CGRectGetMaxY(textQuran.frame) + 5, width - offSetX * 2, CGFloat(height2))
                OptionsView.frame = CGRectMake(0, CGRectGetMaxY(textCn.frame) + 5, OptionsView.frame.size.width, OptionsView.frame.size.height)
//                textQuran.sizeToFit()
//                textCn.sizeToFit()
//                
//                let offSetX : CGFloat = 15
